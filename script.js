function loco() {
  //just for clean code locomotive is wrapper in function for readability
...
gsap.to("#page2>h1>span", {
  scrollTrigger: {
    ...
    //1st property is for element and second property is for screen
    start: `top bottom`,
    //here it means when element's top is in viewport trigger gsap
    // bottom indicates when bottom part of element is visible in viewport
    end: `bottom top`,
    // end the animation when element's bottom is on top of screen
    scroller: `#main`, //which element to scroll to trigger gsap
    scrub: 0.5,
    //plays animation in forward and reverse order
    markers: false, //shows start and end markers of animation
  },
  stagger: 0.2,
  color: `white`,
});
...
document
  ...
  .forEach(function (dets) {
    pg4Clutter += `<span> ${dets} </span>`;

    document.querySelector("#page4>h1").innerHTML = pg4Clutter;
  });
